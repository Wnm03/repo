...
const BusinessFlowPresenter = {
  ...
  render() {
    ...
    el.innerHTML = cards.map((c, i) => `
      <div class="findash-card${c.onClick ? ' u-pointer' : ''}"${c.onClick ? ` data-action="${escapeHtml(c.onClick.action)}" data-args="${escapeHtml(JSON.stringify(c.onClick.args))}"` : ''} aria-label="Buka ${escapeHtml(c.label)}">
        ...
        <div class="findash-card-body">
          ...
          ${(i === 0) ? `<button class="btn btn-sm btn-ghost u-mt6" data-action="BusinessFlowPresenter.openPurchaseOrderBatchModal">🧾 Buat PO Multi-Produk</button>` : ''}
    ...
    // S381: daftar batch PO multi-produk — ikut siklus render() yang sama
    // (pola sama renderTransferList() di atas).
  ...
  currentLocation(productId) {
    ...
    // Manual override (Sesi 376, module Inventory Movement) — kalau user PERNAH
    // pilih lokasi manual lewat renderMovement() (klik salah satu baris rantai),
    // itu SELALU menang di atas derivasi otomatis dari lifecycle/stok di bawah.
    // Disimpan di D.productMovementOverride[productId] (koleksi baru TAPI bukan
    // stok baru — cuma penanda posisi, sama sekali tidak menyentuh
    // D.products[idx].stock). manual:true di hasil balikan dipakai renderMovement()
    // utk menampilkan tombol "Reset ke Otomatis".
    const override = (D.productMovementOverride || {})[productId];
    if (override && override.location) {
      return { ok: true, location: override.location, orderId: null, manual: true, ts: override.ts || null };
    }
    // Inventory Transfer aktif (Sesi 243, D.inventoryTransfers status
    // ON_TRIP) — Sesi 377 (audit sinkronisasi Purchase->Transfer->Sale):
    // sinyal fisik PALING LANGSUNG bahwa produk ini sedang di-rit
    // Magelang->Pekalongan (dicatat createInventoryTransfer(), reuse APA
    // ADANYA — 0 field baru). Sebelum fix ini, currentLocation() TIDAK
    // PERNAH baca D.inventoryTransfers sama sekali, jadi produk yang
    // sedang di-rit tetap salah ditampilkan sbg SUPPLIER (stok 0) atau
    // PEKALONGAN_STORAGE (stok masih ada dari batch lama) — makanya
    // Manual Override (S376) sempat jadi satu-satunya tambalan. Cek ini
    // menang di atas derivasi order/stok di bawah (transfer AKTIF = fakta
    // lapangan real-time), tapi kalah dari Manual Override di atas (user
    // tetap bisa override kalau derivasi manapun meleset). Status
    // RECEIVED sengaja TIDAK dicek di sini — begitu diterima, posisi
    // sudah cukup terwakili lewat derivasi order/stok yang sudah ada.
    const activeTransfer = this._activeTransferForProduct(productId);
    if (activeTransfer) {
      return { ok: true, location: 'ON_MOTOR', orderId: null, transferId: activeTransfer.id };
    }
    ...
    // Purchase Order (Sesi 378, D.purchaseOrders) — lanjutan audit S377:
    // sebelum ini, status SUPPLIER murni TEBAKAN dari stock===0 (0 record
    // pembelian nyata). Sekarang kalau ADA record Purchase Order utk
    // produk ini, dipakai apa adanya (fakta tercatat, bukan tebakan lagi):
    // status RECEIVED (sudah sampai Magelang, belum sempat di-transfer ke
    // Pekalongan) -> MAGELANG_STORAGE — lokasi yg SEBELUM Sesi 378 tidak
    // pernah bisa didapat otomatis sama sekali (cuma lewat Manual Override
    // S376). Status ORDERED (baru dipesan, belum sampai) -> tetap SUPPLIER,
    // tapi sekarang eksplisit dari record, bukan tebakan stok. Cek ini di
    // BAWAH transfer/order-lifecycle (keduanya sinyal fisik LEBIH BARU dari
    // sekadar "sudah dipesan") tapi DI ATAS fallback stok mentah.
    const purchase = this._latestPurchaseForProduct(productId);
    if (purchase) {
      const loc = purchase.status === 'RECEIVED' ? 'MAGELANG_STORAGE' : 'SUPPLIER';
      return { ok: true, location: loc, orderId: null, purchaseId: purchase.id };
    }
  ...
  // Record beli dari supplier yg SEBENARNYA belum pernah ada (lihat audit
  // S377) — MINIMAL: 2 status (ORDERED->RECEIVED), 0 field harga/biaya baru
  // (Purchase estimasi biaya restock TETAP pakai PurchaseEngine.
  // estimatedCost() yg sudah ada, S198 — tidak diduplikasi di sini). Tujuan
  // SATU-SATUNYA: kasih currentLocation() sinyal SUPPLIER/MAGELANG_STORAGE
  // yg nyata (bukan tebakan stok), pola CRUD sama persis
  // createInventoryTransfer()/receiveTransfer() (S243) di atas.

  // createPurchaseOrder({productId,qty}) — catat 1 record pesanan ke
  // supplier. Validasi produk nyata (reuse master Etalase, bukan re-entry
  // manual) + qty>0/finite (pola sama _validateTransferRequest()).
